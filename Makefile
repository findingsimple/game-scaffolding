...
export-all: export-web ## Export Web + Windows + Linux + macOS builds to build/
	...
	$(GODOT) --headless --path . --export-release "Windows Desktop" $(BUILD_DIR)/windows/coin-dash.exe
	$(GODOT) --headless --path . --export-release "Linux/X11" $(BUILD_DIR)/linux/coin-dash.x86_64
	$(GODOT) --headless --path . --export-release "macOS" $(BUILD_DIR)/macos/coin-dash.zip
